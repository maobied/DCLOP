...
#============================================================================== skcubeADCS20170623to20180304+synthetic-anomalies
#****************************************************************************************************
...
class ClusteringLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.input_spec = InputSpec(dtype=K.floatx(), shape=(None, input_dim)) # glorot_uniform
...
class ParametricRelu(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.alpha = self.add_weight(
            name = 'alpha', shape=(input_shape[0]), #edit: 0 instead of 1
            initializer = 'zeros', #'glorot_uniform', #'he_uniform'
            trainable = True 
        )
    ...
    def call(self, x):
        ...
        pos = K.relu(x) #layers.ReLU(x)
        neg = self.alpha * K.relu(-x) ##layers.ReLU(-x)
        return pos + neg
## ---------------------------------------------------------------------------
n_stacks = len(dims) - 1
act = ParametricRelu()
...
## ---------------------------------------------------------------------------
clustering_layer = ClusteringLayer(n_clusters, name='clustering')(encoder1.output) #  (latentnetwork.output) #
...
for ite in range(int(maxiter)):
    ...
    index = index + 1 if (index + 1) * batch_size <= X.shape[0] else 0 #X
...
# #------------------------
# dec_cluster_centroids = dec_cluster_centers #model_weights_1
# dec_cluster_centroids_matrix = dec_cluster_centers #
#-----------------------------------------------------------------------------
...
print("Silhouette Coefficient of deep clustering: %0.3f"
      % metrics.silhouette_score(X_embedding, dec_labels)) #X_embedding
...
###########               Implementing CLOP                      #############
###===========================================================================
dec_clust = loop.LocalOutlierProbability(X_embedding, extent=3, n_neighbors=10, n_neighbors_temp0=10, cluster_labels=list(dec_labels), cluster_labels_temp0=list(dec_labels), cluster_centers = dec_cluster_centers_from_model).fit() 
